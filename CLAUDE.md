...
3. **Placeholders** marked `// TODO(content)` — **seven** categories, complete: WA number, bank account + holder, address + maps coords, photos, logo file, hero copy, **rate card**. `rg "TODO\(content\)"` must find all seven and nothing else. The rate card joined on 2026-08-11 when the client answered _where_ a price appears without supplying _what_ it is. Named `content` not `phase2` because the re-cut made "Phase 2" the landing page; these swap after Phase 4.
