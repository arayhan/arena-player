...
3. **Placeholders** marked `// TODO(content)` — seven categories in the vocabulary: WA number, bank account + holder, address + maps coords, photos, logo file, hero copy, rate card. `rg "TODO\(content\)"` must find only these categories and nothing else — an **allowlist, not a headcount**, because a supplied item loses its marker. **Four are supplied**: the WA number (2026-08-11, `src/modules/home/home.constants.ts`), the **bank accounts** (2026-08-15, `src/server/payment-accounts.ts` — two, both a.n. MARIANA ULFAH), and the **address + maps coords** (2026-08-15 — the client's own embed URL in `LocationBlock.tsx` and the address beside it). The **rate card** arrived the same day and **went back to outstanding when slots became hourly**: the figures priced two-hour blocks and nothing the client said makes them divisible, so `src/server/rates.ts` returns no rates rather than a guessed one. **Four remain outstanding**: rate card, photos, logo file, hero copy — the last of which ships drafted and only becomes a swap if the client wants their own wording. Named `content` not `phase2` because the re-cut made "Phase 2" the landing page.
