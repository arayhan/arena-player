/**
 * The slot vocabulary BOTH surfaces speak.
 *
 * IT LIVED IN `src/modules/home/order.utils.ts` UNTIL 2026-08-15, when the
 * booking form grew a schedule picker of its own. Feature modules never import
 * each other — shared vocabulary moves out or it gets duplicated, and a second
 * copy of `partitionSlots` means the two surfaces can disagree about which
 * hours are still bookable while both look correct.
 *
 * `src/utils/` and not `src/domain/`: domain is byte-identical with
 * arena-player-admin, and the admin app has no slot GRID to display. These are
 * web-only display helpers, which is exactly what this folder is for.
 */
...
/**
 * One row of `GET /api/availability`, structurally.
 *
 * TAKEN AS A SHAPE RATHER THAN AS A NAMED TYPE, so this module imports neither
 * `home.types` nor the booking form's zod-inferred one. Both satisfy it, and
 * neither module has to reach across the boundary to hand its rows over.
 */
 ...
 */
export type DisplayStatus = SlotStatus | "elapsed";
