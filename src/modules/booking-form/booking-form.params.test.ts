...
/** 2026-08-11, 10:00Z = 17:00 Jakarta. */
...
describe("readBookingParams", () => {
  it("accepts a well-formed link for a future slot today", () => {
    expect(readBookingParams("2026-08-11", "20.00 - 22.00", NOW)).toEqual({
      kind: "valid",
      date: "2026-08-11",
      slot: "20.00 - 22.00",
    });
  ...
  it("treats missing params as unusable rather than crashing", () => {
    ...
    expect(readBookingParams(null, "20.00 - 22.00", NOW).kind).toBe("unusable");
    expect(readBookingParams(undefined, undefined, NOW).kind).toBe("unusable");
  });

  it("rejects an unparseable date", () => {
    expect(readBookingParams("11-08-2026", "20.00 - 22.00", NOW).kind).toBe("unusable");
    expect(readBookingParams("besok", "20.00 - 22.00", NOW).kind).toBe("unusable");
    expect(readBookingParams("", "20.00 - 22.00", NOW).kind).toBe("unusable");
  });

  it("REJECTS A NEAR-MISS SLOT FORMAT — the one that would double-book", () => {
    // uniq_active_slot compares time_slot as TEXT. "20.00-22.00" without the
    ...
    for (const near of ["20.00-22.00", "20:00 - 22:00", "20.00 – 22.00", "8pm", "20.00 - 22.00 "]) {
  ...
  it("calls a slot that has already started expired, not valid", () => {
    // 17:00 Jakarta; the 16.00 slot is underway and cannot be booked.
    expect(readBookingParams("2026-08-11", "16.00 - 18.00", NOW).kind).toBe("expired");
  });

  it("calls a date behind the window expired", () => {
    expect(readBookingParams("2026-08-01", "20.00 - 22.00", NOW).kind).toBe("expired");
  });

  it("calls a date past the 14-day window expired", () => {
    expect(readBookingParams("2026-09-30", "20.00 - 22.00", NOW).kind).toBe("expired");
  });
  ...
  it("keeps the date and slot on an expired link so the page can name them", () => {
    ...
    const result = readBookingParams("2026-08-11", "06.00 - 08.00", NOW);
    expect(result).toEqual({ kind: "expired", date: "2026-08-11", slot: "06.00 - 08.00" });
  });
  ...
  it("DOES NOT check availability — that would be a check-then-insert race", () => {
    ...
    expect(readBookingParams("2026-08-11", "20.00 - 22.00", NOW).kind).toBe("valid");
  });

  it("accepts the last slot of the last day in the window", () => {
    // Boundary in the direction that silently loses a booking if wrong.
    expect(readBookingParams("2026-08-24", "22.00 - 24.00", NOW).kind).toBe("valid");
  });
});
