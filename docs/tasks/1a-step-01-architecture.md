...
1. **Date handling** — native `Intl` inside a tested `lib/shared/dates.ts`, or a library. All date logic is Asia/Jakarta, window is today + 13 days, and `toISOString()` is never used for dates. Whatever wins must make `isPastSlot` cover dates _before_ today, not just today's elapsed hours — that was a real bug once.
...
3. **Route split confirmed** — `/` must never load `react-hook-form` or `zod`. Those are `/booking` dependencies and together they are ~22KB gzip, which is more than the entire component headroom. Write down _how_ the split is enforced, not just that it is intended.
