...
**The price is `Harga menyusul`, carrying a `TODO(content)` marker.** Hard rule 2: `/` renders no number of any kind and `/booking` is the exception, but the rate card has still not arrived and **no figure may be invented in the meantime**. The list is built by `buildTimeOptions`, a pure function with its own tests — which hours appear, in what order, which are selectable, which carry a price — because that is the layer that can be wrong without looking wrong.
...
**The payment panel reads its accounts from the API as of 2026-08-15**, and has the four states a fetch has: a skeleton, a list, an empty placeholder, and a failure. The empty state keeps the **dashed box** — Product Principle 7, a placeholder must look like one — while a failed request takes the danger surface and a retry, because "the field has not given us an account yet" and "the network is broken" are different facts and only one of them is worth retrying.
