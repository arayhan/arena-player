...
**The schedule is EDITABLE HERE as of 2026-08-15, and it is the landing page's own plate.** The summary line stays — it answers "what did I pick" once the grid has scrolled past — and under it sits the 14-day date row over the slot grid. `SlotCell` and `DatePills` moved to `src/components/` the same day so both surfaces can render them: a visitor who chose an hour on `/` meets the identical control, in the identical states, rather than a second vocabulary for the same job. Two columns here where the landing plate reaches three, because this grid sits inside a form plate a third narrower and a third column would put the hour, its state and its price into a 200px field.

**Selection is multiple, and the cell needed nothing new to carry it.** `aria-pressed` was already the selection signal, and a toggle is a toggle whether one or four are on. Hours need not be consecutive: a morning game and an evening one on the same day is a real booking.

**The price line is the cell's third row, at `xs`, under the state label** — which hour, is it free, what does it cost, in that order. It renders **only when a `priceLabel` prop is passed**, and `/` passes none: hard rule 2 says the landing page carries no number of any kind, and now that one component serves both surfaces, making the price the caller's to supply is what keeps that enforceable rather than remembered. **The label reads `Harga menyusul` and carries a `TODO(content)` marker** — the rate card has still not arrived, and no figure may be invented in the meantime.

**Two states the picker owns.** A preselected hour that is no longer free is dropped from the selection with an amber line saying so — the cell for a taken hour refuses selection, which also means it refuses DEselection, so without this the payload would carry an hour with no control attached to it. And a 409 now keeps the visitor on this page: the button under it re-reads availability and returns focus to the grid, where everything they typed is still waiting. It used to be a link back to `/#order`, which was the only step in the flow that lost work.
