...
**The selection totals itself.** Under the field, once every chosen hour is priced: `Total sewa` and `DP 50%`, the second in Signal Blue because it is the number a visitor actually transfers. Two hours at 800.000 reads very differently from one row saying 800.000, and the payment copy has promised "Transfer DP 50% dari harga sewa" since Phase 3 opened — until the rate card arrived the page could not keep that promise. **It renders only when the sum is complete**: a partial total is a wrong number somebody transfers against, so an unpriced hour hides the block rather than under-reporting it.
