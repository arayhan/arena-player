...
**The Off-Screen-Is-Off Rule.** Any continuously-running animation must stop when its section leaves the viewport, via `IntersectionObserver`, and must stop under `prefers-reduced-motion` and under `navigator.connection.saveData`. The marquee is currently the only one.
...
- **Ghost numeral:** `24` at `clamp(190px, 44vw, 580px)`, `navy-700` at 70% opacity, `aria-hidden`. It is the only number on this plate that is not a bookable time — the hours the field spans, 06.00 to 24.00. It carries the page's one parallax: it drifts at `plate.offsetHeight * 0.18`, scrubbed to scroll, the way a second board seen past a fence lags the fence in front of it.
