...
- **The scroller is capped at `min(58svh, 420px)` BELOW 980px, and uncapped above it** — split on 2026-08-15. Four month grids unrolled inline measure **1218px**, which on a phone would shove the slot grid off the screen exactly when a visitor went looking for a date; capped, the calendar scrolls inside itself and the grid stays where it was. On a desktop there is no such trade — the plate is wide and the viewport is tall — and a scrollbar inside a panel the page could simply scroll past is a second scrollport a visitor has to notice and aim at. Measured after: **1218px with nothing clipped at 1280**, **418px and scrolling at 390**. The query is the plate's own `@min-[980px]` container query, not a viewport breakpoint, so the calendar changes shape with the object it lives in.
