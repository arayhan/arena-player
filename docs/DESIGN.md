...
**The page-wide scroll reveal is built, and it is opt-in by attribute.** **Sixteen elements carry it**: the three section heading blocks, the order plate, the nine Ketentuan rows, the location block, and the closing heading and lede. Any element carrying `data-reveal` fades and rises 48px over 700ms when its top reaches 88% of the viewport, **fires once and is then unobserved**. `ScrollReveal.tsx` is a single client island rendering no visible DOM: it reads `[data-reveal]` from the document and drives them through `src/lib/motion.ts`, so every section stays a **server component** and ships no JavaScript for the effect. A section added later opts in by writing one attribute.

**Where the marker goes is a composition decision, not a sweep.** A heading and its lede move as ONE object; revealing them separately reads as two decisions where the composition is one. The section ordinal is excluded — it is `aria-hidden` decoration, and animating a watermark draws the eye to the thing that is not the point. The order plate reveals whole, because the date row, the offer panel, the grid and the hand-off band are a single sign. **The Ketentuan rows are the exception and reveal individually**: nine rows crossing the fold one at a time is the page's longest scroll doing something with the distance, and each row carrying its own trigger makes the cascade a property of scrolling rather than a timed stagger that can run ahead of the reader.
...
**The header is fixed, so every anchor jump owes it its own height — and owes it exactly once.** `html { scroll-padding-top: 81px }` is declared on the scroll container and is now the WHOLE offset: `Section` carried a `scroll-mt-4` on top of it until 2026-08-16, and the two summed to a landing nobody could read off one line. Per-section is also how three of four anchors end up correct and the fourth does not.

**81px, and the last term is the one that used to be missing.** 18px top padding + a 44px `size="sm"` CTA + 18px bottom padding + a **1px `border-b`** = 81. The border is present in both header states — transparent at rest, `grey-200` past 40px — so it occupies a row either way, and the value stood at 80px for as long as the arithmetic left it out. There are exactly four in-page anchors (`#order` from the header CTA, the hero CTA and the closing CTA; `#lokasi` from the hero). Verified in the browser rather than derived: header height **81px**, and after a hash jump both `#order` and `#lokasi` land at **exactly 81px** with the header's bottom edge at **81px** — flush, no strip of the previous section showing under a fixed bar. Same at 390px.

> **The offset was 97, then 128, and is now 81.** The 97 was this padding plus `Section`'s own `scroll-mt-4`, which its comment described as unnecessary in the same breath as applying it. Asked to make a landing that felt wrong "scroll further", the padding was raised to 112 — total 128 — which made the gap obvious rather than fixing it. The lesson is not the number: an offset assembled from a global property and a per-component class is one nobody can read, and the second contributor is what the first one's comment already said should not exist. A section's own `--space-section-y` supplies whatever air its heading needs, fluidly, which a fixed 16px margin cannot.
...
Behind a **"Pilih tanggal lain"** disclosure under the pill row.

> **It stopped being the elapsed toggle's twin on 2026-08-15.** It shared that control's caret, muted `sm` label and hover wash on the argument that the plate has one way of saying "there is more behind this" — and the symmetry cost it both things it needed. It looked like the same class of object as a collapsed list of hours nobody can book, so it read as skippable; and it explained nothing. **The pills reach fourteen days and the window is three months**, so a visitor wanting a date in October had no way to learn from those three words that October exists. It now takes the interactive blue, the display face at `label`, a `FiCalendar` mark and a second muted line: _"Tanggal lebih dari 14 hari ke depan bisa dipilih lewat kalender."_ Still a full-width disclosure row rather than a pill (which would read as a third date) or a filled navy bar (which is the hand-off band's device and means "go to WhatsApp" on this plate).
...
- **The scroller is capped at `min(58svh, 420px)` BELOW 980px, and laid out as a 2-column horizontal grid above it** — updated on 2026-08-16. Four month grids unrolled vertically in one column on a phone would shove the slot grid off the screen; capped, the calendar scrolls inside itself. On desktop, the plate is wide and lays out the months in 2 columns (`@min-[980px]:grid-cols-2` with `@min-[980px]:gap-6 @min-[980px]:p-4`), with static headings and natural height. The query is the plate's own `@min-[980px]` container query, not a viewport breakpoint, so the calendar changes shape with the object it lives in.
...
**The schedule is EDITABLE HERE as of 2026-08-15 — two labelled selects, not a plate.** The first version of this brought the landing page's slot grid into the form, and it read as the homepage pasted into a document: a nine-cell plate and a scrolling pill row wedged between a summary and a text input, in a column of ordinary labelled fields. **Tanggal** is a single select over the 14 bookable days; **Jam** is a multi-select. The summary line above them stays, because the fields answer "what can I pick" and only the summary answers "what did I pick" once they have closed.

**The time field is react-select's shape and none of its code.** A control box holding one chip per chosen hour, a dropdown listing the whole day, keyboard navigation throughout — that is what was asked for, and it is ~90 lines here against roughly **35-45KB gzip** for the package plus `@emotion` on a route with **240KB** to spend that has already spent 126.5 on the framework, 63.2 on zod and 17.5 on axios. The package would also have styled itself through emotion objects while every other surface here is a Tailwind v4 token.

**Chips are siblings of the trigger, never nested inside it.** A `<button>` inside a `<button>` is invalid HTML and the browser's own recovery closes the outer one early, breaking both. The box is a plain `div`: chips first, then the trigger stretching across the rest, so a tap on empty space still opens the list.

**Focus never leaves the trigger.** The active option is tracked by index and announced with `aria-activedescendant`. Moving real focus into the list is what makes a multi-select fight the browser after every toggle. Arrows move, Enter or Space toggles, Escape closes and returns focus, Home and End jump, and the list wraps at both ends because nine rows are short enough that stopping dead reads as a jammed control.

**Every hour is listed, including the ones nobody can book.** A dropdown holding only free hours makes a nearly full day look like a day with two hours on offer, and "why is 20.00 missing" has no answer on screen. Taken, pending and elapsed rows stay, dimmed, each carrying its own state word — **and they stay in the arrow order**, so a keyboard visitor reaches 20.00 and hears "Terisi" rather than finding it absent. `aria-disabled`, never the native attribute, exactly as the slot cell does it on `/`.

**Each row is hour · state · price, and it stacks to two lines below `sm`.** Measured at 390px: the three parts need 368px inside a 316px row, so the price was clipped by **50px** — the field whose whole job is answering "what does it cost" losing precisely that answer on the primary device. A taken hour shows an em dash where the price would be, because a blank column reads as a missing value.

**The price is the client's own figure as of 2026-08-15** — `Rp 400.000` before noon, `Rp 600.000` to 18.00, `Rp 800.000` after, per two-hour slot. It replaced the `Harga menyusul` placeholder the day the rate card arrived. Hard rule 2 is unchanged: `/` renders no number of any kind and `/booking` is the exception, and a slot the rate card does not price still shows **no figure at all** rather than a guessed one. The list is built by `buildTimeOptions`, a pure function with its own tests — which hours appear, in what order, which are selectable, which carry a price — because that is the layer that can be wrong without looking wrong.

**The selection totals itself.** Under the field, once every chosen hour is priced: `Total sewa`, and nothing else. It carried a `DP 50%` row beside it until 2026-08-16, when the payment panel's sentence promising a half-deposit was removed — a figure whose only explanation has just been deleted is a number a visitor cannot act on. Two hours at 800.000 still reads very differently from one row saying 800.000, which is why the sum stays. **It renders only when the sum is complete**: a partial total is a wrong number somebody transfers against, so an unpriced hour hides the block rather than under-reporting it.

**Rupiah is formatted through `Intl.NumberFormat("id-ID")`** — zero bytes, and the reason is the separator: Indonesian groups thousands with `.`, where a naive `toLocaleString()` on a machine set to en-US produces `400,000` — the same digits under the wrong convention, on a page whose entire audience reads the other one. No decimals: there are no sen in a field rental.

**Two states the field owns.** A preselected hour that is no longer free is dropped from the selection with an amber line naming it: a taken row refuses selection, which means it also refuses DEselection, so silence would leave a value in the payload with no control on screen able to remove it. And a 409 now keeps the visitor here — the button re-reads availability and returns focus to the field, with everything they typed still in place, where it used to be a link back to `/#order` and the only step in the flow that lost work.

**The payment panel reads its accounts from the API as of 2026-08-15**, and has the four states a fetch has: a skeleton, a list, an empty placeholder, and a failure. **The list is real now** — the client's two accounts, BCA and BRI, both a.n. MARIANA ULFAH. The BRI number keeps the dashes the client writes it with, because that is how a visitor checks it against a statement; the copy button strips them, because a banking app's account field takes digits. The empty state keeps the **dashed box** — Product Principle 7, a placeholder must look like one — while a failed request takes the danger surface and a retry, because "the field has not given us an account yet" and "the network is broken" are different facts and only one of them is worth retrying.

**The skeleton borrows the row's own typography rather than sizing bars by hand.** Hand-sized bars matched the row at 1280px and were **14px short at 390px**, and then the row itself stretched to 170px when the holder's name pushed the button onto its own line. Both now share one `ROW_CLASS` and the same three text classes, with the skeleton rendering placeholder strings tinted out — so the line boxes are identical by construction at any width. **The row also stacks below `sm` for both states**: with `flex-wrap` alone, whether the button sat beside the text or under it depended on how long an account holder's name happened to be, which is not something a loading state can predict. Measured after: 126 against 126 at 1280, 166 against 170 at 390 — the remaining 4px is the sample name wrapping, not the structure.

**The number is set at `h3` with `tabular-nums`, grouped in fours; the copy button copies the RAW digits.** Even digit widths are what let a visitor check a long number against a banking app one group at a time — and Plus Jakarta Sans does ship `tnum`, unlike the display face where the same class was measured to be a no-op. Groups of four rather than per-bank conventions: Indonesian banks number accounts from 10 to 15 digits with their own groupings, and a wrong grouping implies a structure the number does not have. **The clipboard gets no spaces**, because a banking app that rejects `1234 5678 90` gets blamed for spaces this UI added.

**Two fields stopped rendering the same day** — Nomor WhatsApp and the proof dropzone. Both are hidden behind one constant each, not deleted: the visitor arrives through the admin's WhatsApp chat, which already has their number and is where payment proof is handled for now.

**The proof field is a dropzone**, added 2026-08-14, **hidden since 2026-08-15**. Recorded in full because it comes back the moment payment proof moves out of the chat. A square target, **dashed at rest and solid `blue-600` over a `blue-50` wash while a file is over it** — a border weight and fill change rather than colour alone, and the same "this is live, take it" vocabulary the slot grid uses. The label switches from _Tarik gambar ke sini_ to _Lepas di sini_. Dashed is the one border on this plate that is not a rule, and that is deliberate: every other edge is a fixed division, so a broken edge is the only one that reads as _something goes in here_.
