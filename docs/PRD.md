...
| 2   | Order   | Anchor `#order`. Date picker (~3-month window) + time slot grid against the mock. **No pricing shown here** |
...
| #   | Task                     | Notes                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
| --- | ------------------------ | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 1   | Layout                   | Page shell + schedule panel: the summary line over an editable date-and-hour picker (a locked summary card until 2026-08-15)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
| 2   | UI                       | Schedule selects — Tanggal (single) and Jam (multi-hour, chips + dropdown listing every hour with its state and price), Nama Tim / Pemesan, notes, payment info card, honeypot. **Nomor WhatsApp and the proof upload are HIDDEN as of 2026-08-15** — both still exist in the code and the contract behind one constant each                                                                                                                                                                                                                                                                                                                                                                             |
| 3   | Validation               | At least one hour selected; required fields; Indonesian phone format (08xx/62xx) **and** image-only ≤2MB still enforced whenever those two values are present, which is what makes un-hiding either a one-line change                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
...
| 6   | End-to-end journey check | **One continuous flow, plus the pasted-link entry — changed 2026-08-15.** Main path: landing → slot select → `/booking?date=…&time=…` arrives with both params prefilled → fill → submit → success → WhatsApp with the message template. Second entry, still supported and still tested: open `/booking?date=…&time=…` directly, as an admin's pasted link would. Verify all four param states from the Product Spec: valid, missing, expired, unavailable. The WA number is a `TODO(content)` placeholder — do not report that as a failure. **Links pasted into chats before 2026-08-15 carry 2-hour slot strings** and correctly read as `unusable`; that is real customer impact, not a test failure |
...
2. **Order section ("Pesan Lapangan")** — the core of the page, must be reachable within the first 1–2 scrolls on mobile. Anchor id `#order`:
   - Time slot grid: 06.00–07.00 through 23.00–24.00 (1-hour slots, 18 per day, since 2026-08-15 — was 2-hour slots, 9 per day), fetched from `GET /api/availability` — seeded demo data until Phase 4, real Neon data once the backend lands
   ...
   - Free-photographer line: **"Gratis fotografer di 1 jam pertama, 16.00–24.00"**, in its own panel between the date row and the slot grid. It came down from the hero on 2026-08-15, which keeps the two-word "Gratis fotografer": the conditions name hours, so they belong where a visitor is choosing hours
   - Date picker: the next ~3 months (92 days). The first **two weeks** are horizontally scrollable pills; the remaining 78 days sit behind a **"Pilih tanggal lain"** disclosure opening a month calendar inside the plate. **Split 2026-08-15**, when the window grew from 14 days to 92 — 92 pills in one strip is a control whose far end nobody reaches
   - **With a slot selected, the hand-off band links to `/booking?date=…&time=…`** — a same-origin route, label "Lanjut isi data →". **Changed 2026-08-15**; it used to open `https://wa.me/<PLACEHOLDER_NUMBER>?text=<template>` and go no further
   - **WhatsApp moved, it was not removed.** The journey is now slot → `/booking` → fill and upload → submit → WhatsApp, so the admin receives a booking that already exists in the database instead of an opening request they have to transcribe. The prefilled template — "Halo, saya mau booking lapangan Arena Player tanggal {DD MMM YYYY} jam {slot}" — is still the message shape and now belongs to the post-submit step
   - The old one-destination rule and its rationale (on mobile `wa.me` deep-links into the WhatsApp app rather than opening a tab, so pairing it with a same-tab navigation is the combination in-app webviews and popup blockers handle inconsistently) **applied to `wa.me` and does not transfer to a same-origin route** — there is no app takeover to race. It still binds wherever the `wa.me` link is rendered
...
**Two entries, both carrying query params — changed 2026-08-15.** The order section's hand-off band on `/` now links straight here with `date` and `time` already correct, and that is the main path. A link pasted by the admin over WhatsApp (or sent by the bot later) is the second, and it is the one that arrives malformed or stale — so all four param states below stay specified and built. What changed is that they are no longer the _only_ case; what did not change is that a blank form or a crash is still the one response this route may never give. Handle all four:
...
| Date outside the ~3-month window, or a slot already past | Same treatment as missing — the link has expired, say so plainly                                        |
...
- Reads `date` and `time` from query params as a **starting selection**, then lets the visitor change both in the form
  - **The summary was locked until 2026-08-15**, and "Ubah jadwal" sent them back to `/#order` — a full page away from a form they had already begun, losing whatever they had typed. The schedule is now **two labelled selects on the form itself**: Tanggal (single) and Jam (multi). A first version reused the landing page's slot grid and was replaced the same day — a nine-cell plate between a summary and a text input reads as the homepage pasted into a form
  - **One booking may cover SEVERAL hours**, consecutive or not. The payload field is `slots`, repeated once per hour; see [architecture.md](architecture.md) for the wire shape and [database.md](database.md) for the one-row-per-slot transaction Phase 4 owes
  - **An expired link no longer dead-ends.** It renders the notice AND the form, opened on that link's date with no hour selected — an expired link is exactly when a visitor needs to pick a new time. A link whose params cannot be read at all still gets the notice alone: there is no date to open on
  - **A preselected hour that is no longer free is dropped from the selection**, with a line saying so. It is not a check-then-insert: nothing is refused, the database stays the authority through the 409
- Fields: **Nama Tim / Pemesan** (required), ~~Nomor WhatsApp~~ (**hidden 2026-08-15** — the visitor arrives through the admin's WhatsApp chat, which already carries their number; still accepts `08xx` / `62xx` / `+62xx` and still **normalised to `628xxxxxxxxx` before storage** if the field returns), notes (optional, **max 500 characters**)
  - **The label gained "/ Pemesan" on 2026-08-15.** "Nama Tim" alone left a visitor booking a casual game unsure whose name was wanted; the field is the name the admin messages on WhatsApp, which is a person about as often as it is a squad. **The payload key stays `teamName` and the column stays `team_name`** — they are the API contract and are shared with the admin repo, so the column name is now a partial misnomer by choice rather than by oversight.
- Payment info card: the account list **read from `GET /api/payment-accounts`** since 2026-08-15. **The instruction text was removed on 2026-08-16** — it read "Transfer DP 50% dari harga sewa. Nominal dikonfirmasi admin via WhatsApp." and had stood as the stand-in for a rate card that never arrived in a usable form
  - Four states: skeleton while loading, one row per account (bank, number grouped in fours, holder, and a **Salin** button copying the raw digits), the placeholder sentence when the list is empty, and a failure message with a retry when the request itself failed. The last two are different facts and read differently.
  - **The list is empty today** — the client has supplied no account — so what ships is the placeholder sentence. The difference from before is that emptiness is now reported by the server rather than typed into a component
  - **RESOLVED 2026-08-11 — a real rupiah amount renders here.** The client's answer: no price on `/`, the price appears in the form. `CLAUDE.md` hard rule 2 has narrowed accordingly, from "no prices anywhere" to "no prices on `/`". **The answer named a surface, not a referrer** — it was recorded as "once the visitor has arrived through the WhatsApp link", which stopped being true on 2026-08-15 when `/` began linking here directly. The rule is unchanged.
  - **The rate card arrived 2026-08-17** as a live, per-hour, weekday/weekend/holiday-aware `rate_card` table (36 rows, cross-checked against the client's own pricelist image) — `src/server/rates.ts` reads it directly via `GET /api/rates?date=…`. It is no longer `TODO(content)`. **No placeholder number is ever invented** — that discipline stays even though a real card exists now, since a future gap in the table (a slot with no row) still renders unpriced rather than guessed.
  - That fallback sentence was removed on 2026-08-16 and is not restored — the real per-slot price and total now render in its place.
...
- Availability API: given a date, return the 18 slots with computed status. Cache no longer than 30s.
...
`POST /api/bookings` is public and unauthenticated by design — there is no auth in this repo. The honeypot stops naive bots and nothing else. Without the following, a trivial script can hold **all 1,656 slots** (18 × 92 days) in `pending` until lazy expiry releases them 24h later, and can burn unbounded R2 storage with 2MB uploads:
...
- ~~Rate card~~ — **SUPPLIED 2026-08-17.** A live, per-hour, weekday/weekend/holiday-aware `rate_card` table (36 rows), cross-checked against the client's own pricelist image and read by `src/server/rates.ts` via `GET /api/rates?date=…`. Added 2026-08-11 when the client settled _where_ a price appears; the _what_ arrived hourly rather than a flat rate. **This was the one placeholder that must never get a stand-in number** — the other six are inert if they ship by accident, an invented price is not — and that discipline is unchanged now that the real figures exist: a gap in the table still renders unpriced, never guessed.

## Binding clarifications

- Package manager: pnpm. Lockfile `pnpm-lock.yaml`; never commit `package-lock.json`.
- Date window: today + 91 days (~3 months — see `BOOKING_WINDOW_DAYS` in `src/domain/dates.ts` for why 92, not a round 90), timezone Asia/Makassar (WITA — the field is in Lombok). Today's slots whose start time has passed render disabled (visible, not hidden).
...
- [x] Form validates every field, including Indonesian phone format and ≤2MB image-only upload — verified in-browser: "Nama minimal 2 karakter" (verified as "Nama tim minimal 2 karakter"; the copy dropped "tim" on 2026-08-15 with the label), "Nomor tidak valid", and "Format harus JPG, PNG, atau WEBP" all render and the submit is blocked
