...
| 2   | UI                       | Schedule picker (multi-hour), Nama Tim / Pemesan, notes, payment info card, honeypot. **Nomor WhatsApp and the proof upload are HIDDEN as of 2026-08-15** — both still exist in the code and the contract behind one constant each                                                                                                                                                                                                                        |
...
- Reads `date` and `time` from query params as a **starting selection**, then lets the visitor change both in the form
  - **The summary was locked until 2026-08-15**, and "Ubah jadwal" sent them back to `/#order` — a full page away from a form they had already begun, losing whatever they had typed. The picker is the landing page's own slot grid and date row (`src/components/`), so both surfaces show identical states
