...
Today the exchange starts with "jam berapa yang kosong?" and takes a round trip before anything is decided. The product moves that question to a page that can answer it, so the WhatsApp conversation begins at "I have booked 18.00, here is the proof" instead. **Changed 2026-08-15**: it used to begin at "I want 18.00", because the site handed the visitor to WhatsApp the moment they picked a slot. The site now carries them all the way to `/booking` and takes the details itself, so what reaches the admin is a submitted booking rather than an opening request.
...
- **Eighteen 1-hour slots per day, 06.00–24.00. Booking window is today + 91 days (92 inclusive).** Both changed 2026-08-15; this line read "nine 2-hour slots" and "today + 13 days" until then.
...
- **Demand is same-day and next-day.** The window is 92 days, but days 3 onward are rarely the reason someone opens the page. At 375px the date row is the first thing in the order section, so ninety-two equal-weight pills would spend the scarcest horizontal space on the least-used dates. **Acted on 2026-08-15**: the row carries the first fourteen days as pills and the remaining seventy-eight sit behind a "Pilih tanggal lain" disclosure opening a month calendar — the far end of the window costs the same two taps as the near end without ever competing for the row.
- **The booking journey no longer breaks in the middle — changed 2026-08-15.** It used to: the site handed off to WhatsApp at slot selection, and the `/booking` link came back through WhatsApp, typed by the admin. That made `/booking` reachable only by a pasted link and put a human transcription step inside the conversion path. The journey now runs **slot → `/booking` → fill and upload → submit → WhatsApp**. WhatsApp is not removed; it moved to the far side of the database write, where it does what it is actually for — a person confirming a booking that already exists. **A pasted link is still a supported entry**, so every malformed and stale param state stays specified and built; it is no longer the only way in.
...
- **Settled 2026-08-11 — pricing.** The client's answer: **no price on `/`, a real rupiah amount on `/booking`**, shown once the visitor has left the landing page for the form. `/booking` is now an explicit exception to the no-prices rule; the landing page is not. **The condition was written as "arrived through the WhatsApp link" and that wording expired on 2026-08-15**, when `/` began linking to `/booking` directly — but the rule it encoded did not change, because what the client settled was _which surface_ carries a number, not which referrer. **The rate card is still outstanding** — the client said where a price goes, not what it is — so the figure is `TODO(content)` and must not be invented in the meantime.
...
- **Superseded — the typefaces have now changed twice, and this line used to be the reason they could not.** It read "Orbitron for display type, Inter for body", and both `DESIGN.md` and the design detector's suppression record cited it as a closed client commitment. It went to Saira on 2026-08-12, and the pair is now **Panchang for display, Plus Jakarta Sans for body** (2026-08-13, the user's own pick). The original ask was NV Dune Hero, which is licensed for personal use only and cannot ship on a commercial site; every substitute since has honoured the same brief — wide, geometric, athletic — and been chosen by measuring the headline against the 343px content box rather than by resemblance. **Panchang loads at 500/700/800 and has no width axis**, which retired the `font-stretch: 125%` the previous face carried. **The client has not seen either change**, and both join the velocity direction in the re-approval that is already owed.

  **Two typeface changes in two days is the argument for the guard, not against the changes.** Each one left six authority documents asserting a face the build had replaced, invisibly, because `check:docs` compared only hex colours and contrast ratios. `pnpm check:docs` now carries `typeface-drift`, which reads the loaded families out of `src/app/layout.tsx` and fails any current-tense claim that disagrees.
