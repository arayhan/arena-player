...
// MSW IS RETIRED — 2026-08-15 — AND THIS BAN OUTLIVES IT ON PURPOSE.
//
// It used to guard `@/mocks/*`: a component importing the fixture as a
// convenient stub would have shipped deterministic fake availability to
// production, rendering perfectly, with every check green and check:budget
// unbothered because the mock was small. PRD.md called that the single most
// likely way this project ships a broken deploy.
//
// The folder is gone — real route handlers in src/app/api/ serve the demo now —
// so the import it guarded cannot be written. The PACKAGE ban stays: reaching
// for msw again would rebuild the service-worker hazard the retirement removed,
// and a rule that disappears with the thing it guarded is a rule nobody
// reinstates in time.
const MOCKS = {
  group: ["msw", "msw/*", "@/mocks", "@/mocks/*"],
  message:
    "MSW was retired on 2026-08-15 in favour of real route handlers in src/app/api/. A service worker in a production build serves invented data with nothing in the console and nothing in the network tab that looks wrong. Availability comes from the route; test fixtures belong in the test file.",
};
...
const eslintConfig = defineConfig([
  ...
  // a task.
  //
  // Test files keep the looser set: they legitimately reach for zod, axios and
  // react-hook-form to exercise the modules that own them, and nothing under
  // *.test.ts is ever bundled. The src/mocks/ half of this exemption went with
  // MSW on 2026-08-15.
  {
  ...
  // public/ is static assets served as-is, not source — linting it would report
  // on files nobody here writes by hand.
