...
- R2 bucket is PRIVATE: `proof_url` stores the object KEY, never a public URL. No presigned URLs in Phase 1.
...
- Package manager pnpm. Shareable code in `lib/`; `lib/` never imports from `app/`; siblings import each other with relative paths + explicit `.ts` extensions (not `@/`).
- Every non-trivial function gets a `scripts/check-lib.ts` assertion.
