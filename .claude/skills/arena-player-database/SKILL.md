...
- Every non-trivial function under `src/` gets a colocated Vitest `*.test.ts` (`pnpm check:unit`, glob `src/`). The live Neon + R2 preflight is `scripts/check-setup.test.ts` (`pnpm check:setup`, Phase 4) — keep it under `scripts/` so `check:unit` never needs credentials.
