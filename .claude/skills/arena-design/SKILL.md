...
- **AI imagery may be abstract/decorative only** (texture, gradient mesh, grain, pattern). Never anything a customer would read as a photo of the actual field — that misleads someone into booking a facility they have not seen. Real venue photos come from the client, `TODO(phase2)`.
